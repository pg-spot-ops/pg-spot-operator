...
def write_action_completed_marker_file(
    host: str,
    login: str,
    private_key_file: str = "",
    max_wait_seconds: int = 10,
) -> bool:  # TODO move to paramiko?
    logger.info(
        "Writing an 'action completed' marker file to %s for %s  ...",
        ACTION_COMPLETED_MARKER_FILE,
        host,
    )
    try:
        ...
        cmd_args = ["touch", ACTION_COMPLETED_MARKER_FILE]
        if private_key_file:
            ssh_args += ["-i", private_key_file]
        rc: int = 0
        start_time: float = time.time()
        ...
        while time.time() < start_time + max_wait_seconds:
            try:
                ...
                logger.debug(
                    "Marker set try %s retcode: %s, output: %s",
                    try_count,
                    rc,
                    output,
                )
                ...
                logger.debug(
                    "Marker set try %s NOK with retcode: %s. Sleeping %ss",
                    try_count,
                    rc,
                    loop_sleep_seconds,
                )
        ...
        logger.error(
            "Failed to set ACTION_COMPLETED_MARKER_FILE %s on %s: %s",
            ACTION_COMPLETED_MARKER_FILE,
            host,
            e,
        )
...
def check_setup_completed_marker_file_exists(
    host: str,
    login: str,
    private_key_file: str = "",
    ...
    try:
        ...
        logger.error(
            "Failed to read marker %s on %s: %s",
            ACTION_COMPLETED_MARKER_FILE,
            host,
            e,
        )
